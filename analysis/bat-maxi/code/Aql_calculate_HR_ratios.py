...
def get_bin_indexes(arr):
    ...
    # Only use bins with 2 3-sigma detections
    k = 0
    while k < len(arr[0]):

        snr_thresh = 3.0
        
        # Only use days where both exhibit 3-sigma detections
        if arr[3][k]/arr[4][k] >= snr_thresh and arr[1][k]/arr[2][k] > snr_thresh:
            left_index.append(k)
...
def plot_binned_lc(group_array, matched_array, l, r):
    ...
    ax.legend()
    plt.savefig('../plots/Aql_HR_lc_Binning.png')
    plt.clf()
    plt.close()
...
def main():
    ...
    bat = np.loadtxt('../AqlX1_watchdog/BATObs/AQLX1/AQLX1_BAT_lc_15_50_crab.txt')
    maxi = np.loadtxt('../AqlX1_watchdog/MAXIObs/AQLX1/AQLX1_MAXI_lc_4_10_crab.txt')
    ...
    bat_filtered = filter_times(bat,ti,tf)
    bat_filtered = filter_errors(bat_filtered)
    maxi_filtered = filter_times(maxi,ti,tf)
    maxi_filtered = filter_errors(maxi_filtered)

    # Save the filtered light curves
    np.savetxt('../results/Aql_filtered_bat.txt',  bat)
    np.savetxt('../results/Aql_filtered_maxi.txt', maxi)
    ...
    np.savetxt('../results/Aql_HR_fits.txt', np.array([group_array[0], HR, HR_neg, HR_pos, group_array[2]]).T)
